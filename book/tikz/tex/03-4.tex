...
\begin{tikzpicture}[
  ...
  \coordinate (s)  at ($(vF)-(vG)$); % symetrique de v : v_F - v_G
  ...
  % Parallelogramme du symetrique
  \draw[dashed,thick] (vF) -- (s);

  % Vecteurs
  \draw[vect] (O) -- (vF) node[below] {$v_F$};
  \draw[vect] (O) -- (vG) node[above left] {$v_G$};
  \draw[vect] (O) -- (v)  node[above right] {$v = v_F + v_G$};
  \draw[vect] (O) -- (s)  node[below right] {$s_F(v) = v_F - v_G$};

  % Origine
  \fill (O) circle (1.8pt) node[below right] {};
\end{tikzpicture}
\end{document}
